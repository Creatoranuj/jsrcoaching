...
import { ChapterFolderViews } from "../components/course/ChapterFolderViews";
...
const LectureListing = () => {
  ...
  // Remember the user's view choice across navigations (same as ChapterView).
  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    const saved = typeof window !== "undefined" ? safeGet("lecturelisting:view") : null;
    return (saved as ViewMode) || "list";
  });
  useEffect(() => {
    safeSet("lecturelisting:view", viewMode);
  }, [viewMode]);
  // Lessons that belong directly to the open chapter (not to its sub-chapter
  // folders) — shown under the folder grid so nothing is hidden.
  const directLessons = useMemo(
    () => lessons.filter((l: any) => !chapterId || l.chapter_id === chapterId),
    [lessons, chapterId],
  );
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (chapterId && chapterId !== "__all__") {
          ...
          // firing the query with a non-uuid returns 400s and a blank screen.
  ...
  useEffect(() => {
    if (!courseId) return;
    // Skip quiz queries for invalid chapter deep links (non-uuid → 400s).
    if (chapterId && chapterId !== "__all__" && !UUID_RE.test(chapterId)) return;
  ...
  if (chapterMissing) {
    return (
      <div className="min-h-dvh flex flex-col items-center justify-center gap-3 bg-background px-6 text-center">
        <p className="text-muted-foreground font-medium">Chapter not found</p>
        <p className="text-sm text-muted-foreground/70">Yeh chapter link valid nahi hai.</p>
        <button
          onClick={() => navigate(`/classes/${courseId}/chapters${fromParam ? `?from=${fromParam}` : ""}`)}
          className="mt-1 min-h-[44px] px-5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold"
        >
          Back to chapters
        </button>
      </div>
  ...
  return (
    <div className="min-h-dvh bg-background nb-smooth-scroll">
      ...
      {/* Sub-chapters section — respects the selected view (list / gallery / table) */}
      {showSubChapters && subChapters.length > 0 && (
        <div className="px-5 py-3">
          ...
          <ChapterFolderViews
            folders={subChapters}
            view={viewMode}
            onOpen={(sc) => navigate(`/classes/${courseId}/chapter/${sc.id}${fromParam ? `?from=${fromParam}` : ""}`)}
          />
          {directLessons.length > 0 && (
            <div className="mt-5 border-t pt-4">
              <h3 className="text-sm font-medium text-muted-foreground mb-3">Lectures</h3>
              {viewMode === "table" ? (
                <LectureTableView
